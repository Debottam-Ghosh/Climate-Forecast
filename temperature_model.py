...
def run_temperature_kalman(temp, months, years, train_mask,
    ...
    if forecast_horizon is not None:
        ...
        future_temp = list(temp_tr[-12:])

        future_features = []

        future_months = [
            (months[train_mask][-1] + i + 1) % 12
        ...
        for i in range(forecast_horizon):
            ...
            lag1 = future_temp[-1]
            lag2 = future_temp[-2]
            lag12 = future_temp[-12]

            roll3 = np.mean(future_temp[-3:])
            roll6 = np.mean(future_temp[-6:])
            ...
            pred_temp = np.clip(pred_temp, -5, 50)
            
            future_temp.append(pred_temp)

        X_te = np.array(future_features)

        y_te = np.full(forecast_horizon, np.nan)
    ...
    # ── stochastic sampling paths ──
    std_est = (p95 - p05) / 3.29

    paths = np.random.normal(
        loc=mean,
        scale=std_est,
        size=(n_paths, len(mean))
    )
