...
def hargreaves_pet(temp_mean, temp_min, temp_max, lat_deg):
    """
    Hargreaves PET (mm/day → converted to mm/month)

    PET = 0.0023 * Ra * (Tmean + 17.8) * sqrt(Tmax - Tmin)

    Ra = extraterrestrial radiation (approx using latitude)
    """
    n = len(temp_mean)

    # Approx Ra (MJ/m²/day) simplified using latitude
    lat_rad = np.radians(lat_deg)
    Ra = 15.392 * (1 + 0.033 * np.cos(2 * np.pi * np.arange(n) / 12))

    delta_T = np.maximum(temp_max - temp_min, 0.1)

    pet_day = 0.0023 * Ra * (temp_mean + 17.8) * np.sqrt(delta_T)

    # convert to mm/month
    days = _DAYS_IN_MONTH[np.arange(n) % 12]
    return pet_day * days
...
def run_drought_model(R, T, temp_min, temp_max, scale=3, lat_deg=17.0):
    """
    ...
    # Training PET
    pet_tr = hargreaves_pet(
        temp_tr,
        temp_min[:len(temp_tr)],
        temp_max[:len(temp_tr)],
        lat_deg
    )

    # Forecast PET (use forecast mean temp)
    pet_fc = hargreaves_pet(
        temp_mean_fc,
        temp_min[len(temp_tr):],
        temp_max[len(temp_tr):],
        lat_deg
