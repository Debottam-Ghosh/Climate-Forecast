...
def thornthwaite_pet(temp_C, months_0idx, lat_deg):
    ...
    months_0idx = np.asarray(months_0idx)
...
def drought_probabilities(index_ensemble):
    """
    Compute monthly drought probabilities from ensemble.
    Three EXCLUSIVE categories (sum to 100% per month):

      no_drought : SPEI/SPI >= -1.0
      moderate   : -2.0 < SPEI/SPI <= -1.0
      extreme    : SPEI/SPI <= -2.0
    ...
    Returns
    -------
    dict with keys 'no_drought', 'moderate', 'extreme' each (T_te,)
    """
    valid   = ~np.isnan(index_ensemble)
    n_valid = np.maximum(valid.sum(axis=0), 1)

    return dict(
        no_drought = (index_ensemble >= -1.0).sum(0) / n_valid * 100,
        moderate   = ((index_ensemble < -1.0) &
                      (index_ensemble >= -2.0)).sum(0) / n_valid * 100,
...
def index_fc_stats(ensemble, representative='mean'):
    """Summary statistics from index ensemble (n_paths × T), ignoring NaN."""
    rep_key = 'med' if str(representative).lower() in ('median', 'med') else 'mean'
    return dict(
        ...
        rep  = np.nanmedian(ensemble, axis=0) if rep_key == 'med' else np.nanmean(ensemble, axis=0),
        rep_label = 'Median' if rep_key == 'med' else 'Mean',
    )
...
def run_drought_model(R, T, scale=3, lat_deg=17.0, representative='mean'):
    """
    ...
    dict with keys:
      ...
      spi_prob      dict             — P(no_drought/moderate/extreme) per month
    ...
    rain_rep    = np.asarray(R.get('fc', {}).get('rep', R['fc']['mean']), dtype=float)
    mo_tr       = R['mo_tr']                      # (T_tr,) 0-indexed
    mo_te       = R['mo_te']                      # (T_te,) 0-indexed
    T_tr        = R['T_tr']
    T_te        = R['T_te']

    use_rep_rain = str(representative).lower() in ('median', 'med')
    ...
    temp_te = np.asarray(temp_te, dtype=float)

    future_mode = (
        len(temp_te) == 0 or
        np.all(np.isnan(temp_te))
    )
    ...
    pet_paths = np.array([
        thornthwaite_pet(path, mo_te_t, lat_deg)
    ...
    rain_rep_mm = rain_rep * days_te
    

    # ── PET for training, observed test, and forecast ────────────────────────
    print(f"  [Drought] Computing Thornthwaite PET (lat={lat_deg}°N) ...")
    pet_tr      = thornthwaite_pet(temp_tr,      mo_tr_t, lat_deg)  # (T_tr,) training

    # Water balance D = rain - PET (mm/month)
    d_tr        = rain_tr_mm - pet_tr   # (T_tr,) training
    ...
    if not future_mode:
        ...
        pet_obs = thornthwaite_pet(
            temp_te,
            lat_deg,
            mo_te
        )

        d_te = rain_te_mm - pet_obs

    else:

        pet_obs = np.full(T_te, np.nan)

        d_te = np.full(T_te, np.nan)
    ...
    # ── statistics ────────────────────────────────────────────────────────────
    spi_fc   = index_fc_stats(spi_ensemble, representative=representative)
    spei_fc  = index_fc_stats(spei_ensemble, representative=representative)
    spi_prob = drought_probabilities(spi_ensemble)
    spei_prob= drought_probabilities(spei_ensemble)

    # ── summary print ─────────────────────────────────────────────────────────
    print(f"  [Drought] SPI-{scale}  {spi_fc['rep_label'].lower()}={np.nanmean(spi_fc['rep']):.2f}  "
          f"P(moderate)={spi_prob['moderate'].mean():.0f}%  "
          f"P(extreme)={spi_prob['extreme'].mean():.0f}%")
    print(f"  [Drought] SPEI-{scale} {spei_fc['rep_label'].lower()}={np.nanmean(spei_fc['rep']):.2f}  "
          f"P(moderate)={spei_prob['moderate'].mean():.0f}%  "
          f"P(extreme)={spei_prob['extreme'].mean():.0f}%")
    ...
    return dict(
        ...
        spi_ctx       = spi_ctx,
        spei_ctx      = spei_ctx,
        pet_mean_fc = np.mean(pet_paths, axis=0),
        mo_te         = mo_te,
        mo_ctx        = mo_ctx,
        ctx           = ctx,
        scale         = scale,
...
def plot_drought(D, district, x_fc, xt, xl, yr_te, mo_te,
    ...
    """
    Plot 3: SPEI forecast ensemble with 3 drought category bands
    ...
    # Three exclusive drought categories for SPEI color bands
    CAT_COLORS = {
        'extreme':    ('#8B0000', 0.18),
        'moderate':   ('#F0A500', 0.15),
        'no_drought': ('#1D9E75', 0.08),
    }

    fig = plt.figure(figsize=(15, 6))
    fig.suptitle(
        f"{district} - Figure 3: All simulated paths for SPEI-{scale}",
    ...
    def _draw_category_bands(ax, xlim):
        """Draw three horizontal SPEI drought category bands on axis."""
        bands = [
            (-3.5, -2.0, 'extreme',    'Extreme'),
            (-2.0, -1.0, 'moderate',   'Moderate'),
            (-1.0,  3.5, 'no_drought', 'No drought'),
        ]
    ...
    def _draw_index_panel(ax, ens, fc, obs, ctx_vals,
        ...
        ax.fill_between(x_fc, fc['p05'], fc['p95'],
                alpha=0.18, color=color, label='5th–95th (90% CI)')
        ax.fill_between(x_fc, fc['p10'], fc['p90'],
                alpha=0.28, color=color, label='10th–90th (80% CI)')
        ...
        if ctx_vals is not None:
            ...
            if valid_c.any():
                ax.plot(x_ctx[valid_c], ctx_vals[valid_c],
                        ...
                        label=f'Historical SPEI-{scale}')

        ax.axvline(-0.5, lw=1.0, color='black', alpha=0.2, ls=':')

        # Ensemble mean
        rep_series = fc.get('rep', fc['mean'])
        valid_m = ~np.isnan(rep_series)
        ax.plot(x_fc[valid_m], rep_series[valid_m],
            lw=2.5, color=color, zorder=5,
            label=f"{label} {fc.get('rep_label', 'Mean').lower()}")
    ...
    for yc, lbl in [(2.6, 'No drought'), (-1.0, 'Moderate'), (-2.0, 'Extreme')]:
