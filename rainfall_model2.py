...
run_rainfall_kalman(rain, months, years, train_mask, n_paths, seed,
                    k_mu, k_sig, q_mu0, q_gamma, representative='mean') -> dict
...
def compute_metrics(obs, fc):
    """RMSE, MAE, BIAS, SKILL, 90% CI coverage."""
    m     = fc.get('rep', fc['mean'])
...
def run_rainfall_kalman(rain, months, years, train_mask,
    ...
    rain_tr = rain_d[train_mask];  
    log_tr  = log_d[train_mask];   
    ...
    if forecast_horizon is None:
        ...
        rain_te = rain_d[~train_mask]
        log_te  = log_d[~train_mask]
        mo_te   = months[~train_mask]
        yr_te   = years[~train_mask]

    else:

        rain_te = np.full(forecast_horizon, np.nan)
        log_te  = np.full(forecast_horizon, np.nan)

        last_month = int(months[train_mask][-1])
        ...
        last_year = int(years[train_mask][-1])
    ...
    yr_te = np.array(yr_te)

    # =========================================================
    # TRUE FUTURE FORECAST HORIZON
    # =========================================================

    if forecast_horizon is None:
        T_te = int((~train_mask).sum())
    else:
        T_te = int(forecast_horizon)
    ...
    s_end = kf['s_filt'][T_tr - 1]
    P_end = kf['P_filt'][T_tr - 1]
    ...
    if forecast_horizon is None:
        ...
        enso_te = np.zeros(forecast_horizon)

        for t in range(1, forecast_horizon):
            enso_te[t] = 0.7 * enso_te[t-1] + np.random.normal(0,0.5)
    ...
    fc['rep'] = fc[rep_key]
    fc['rep_label'] = 'Median' if rep_key == 'med' else 'Mean'
    ...
    if acc is not None:
        ...
        print(
            f"  [Rain] Future forecast generated "
            f"({T_te} months)"
        )
